...
public class StreamingQuote {
    ...
    /**
     */
    public ListenableFuture<List<Quote>> stream(
            final StreamHandler<Quote> handler, String... symbols) {
        ConsumerKey consumer =
                new ConsumerKey(
                        ForemanConstants.API_KEY.toString(),
                        ForemanConstants.API_SECRET.toString());
        RequestToken user =
                new RequestToken(
                        ForemanConstants.ACCESS_TOKEN.toString(),
                        ForemanConstants.ACCESS_TOKEN_SECRET.toString());
        OAuthSignatureCalculator calc = new OAuthSignatureCalculator(consumer, user);

        ListenableFuture<List<Quote>> response =
                client.prepareGet(
                                APICall.getStreamingQuote(ResponseFormat.XML)
                                        + getParameters(symbols))
                        .setSignatureCalculator(calc)
                        .execute(
                                new AsyncHandler<List<Quote>>() {
                                    private List<Quote> quotes = new ArrayList<Quote>();

                                    @Override
                                    public AsyncHandler.STATE onStatusReceived(HttpResponseStatus s)
                                            throws Exception {
                                    ...
                                    public AsyncHandler.STATE onHeadersReceived(
                                            HttpResponseHeaders bodyPart) throws Exception {
                                        return AsyncHandler.STATE.CONTINUE;
                                    }
                                    ...
                                    public AsyncHandler.STATE onBodyPartReceived(
                                        ...
                                        if (q.equals(
                                                "<status>connected</status>")) { //TODO make more robust
                                            return AsyncHandler.STATE.CONTINUE;
                                        }

                                        Quote quote =
                                                JAXBUtils.getElement(
